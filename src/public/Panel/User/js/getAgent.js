...
function autocomplete(inp, arr) {
    ...
    xhr.addEventListener("readystatechange", function () {
        ...
        if (this.readyState === 1) {
            ...
            getAgent = JSON.parse(this.responseText);
            if (getAgent == null) {
            }
            else {
                for (x in getAgent) {
                    console.log(getAgent[x].ACC_NAME);
                    airports[x] = getAgent[x].ACC_NAME;
                    console.log("agents are" + airports[x]);
                }
            }
        }
        ...
        for (i = 0; i < arr.length; i++) {
            ...
            if (arr[i].substr(0, val.length).toUpperCase() == val.toUpperCase()) {
                ...
                b.addEventListener("click", function (e) {
                    ...
                    console.log("The Selected value is: " + this.getElementsByTagName("input")[0].value);
                    localStorage.setItem("ACC_NAME", this.getElementsByTagName("input")[0].value);

                    var name = localStorage.getItem("ACC_NAME")
                    console.log(getAgent);
                    for (x in getAgent) {
                         
                        if (getAgent[x].ACC_NAME == name) {
                            console.log("the gst number is: " + getAgent[x].GST_NUMBER);
                            localStorage.setItem("GST", getAgent[x].GST_NUMBER);
